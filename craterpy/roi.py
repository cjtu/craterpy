...
class CraterRoi(np.ndarray): # Inheriting from ndarray doesn't work, rethink
    ...
    """
    def __init__(self, lat, lon, rad, wsize=2, nodata=None, plot=False):
        pass
    ...
    @property
    def mask(self):
        """Return array of 0 where not masked, 1 where masked."""
        return np.zeros(self.shape)[self.__masked_where]

    @mask.setter
    def mask(self, roi_mask):
        self.__mask_where = np.where(roi_mask == 1)
